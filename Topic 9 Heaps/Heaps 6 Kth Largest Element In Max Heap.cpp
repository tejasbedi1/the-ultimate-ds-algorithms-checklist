...
class heap
{
	...
	void findKthLargest(int k)
	{
		...
		while (k > 1)
		{
			...
			pq.pop();
			if (childPos_1 != -1)
				pq.push( make_pair (a[childPos_1],childPos_1) );
			if (childPos_2 != -1)
				pq.push( make_pair(a[childPos_2],childPos_2) );
...
{	
	...
	cout<<"Kth Largest Procedure Starting | ";
	heap.findKthLargest(5);
}

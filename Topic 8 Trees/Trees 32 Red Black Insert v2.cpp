...
void insertIntoRBRTreeRecur(nodeRB* &head, nodeRB* &node, int data)
{
	...
	if (node->data > data)
	{
		...
		else
		{
			...
			bool wereThereViolations = checkViolations(head, node->left);
			while (wereThereViolations == true)
			{
				cout<<"NODE->LEFT "<<node->left<<" | ";
				wereThereViolations = checkViolations(head, node->left);
			}
	...
	else if (node->data < data)
	{
		...
		else 
		{
			...
			bool wereThereViolations = checkViolations(head, node->right);
			while (wereThereViolations == true)
			{
				cout<<"NODE->LEFT "<<node->right<<" | ";
				wereThereViolations = checkViolations(head, node->right);
			}
		}
	}
}

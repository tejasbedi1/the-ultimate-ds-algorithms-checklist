...
int maxProfit(int k, vector<int>& prices) 
{
    ...
    for (i=0; i<prices.size()-1; i++)
    {
        ...
        else if (prices[i] < prices[i+1])
        {
            ...
            transactions++;
            vec.push_back( make_pair(init,prices[i]) );
        }
    }
    ...
    for (i=0; i<transactions-k; i++)
    {
        ...
        {		
            if (minLoss > (vec[j].second - vec[j].first))
            {
                minLoss = (vec[j].second - vec[j].first);
                minIndex = j;
                flag = 1;
            }	
            if (vec[j].first <= vec[j+1].first)
            {
                loss = 	(vec[j].second - vec[j].first) + 
                        (vec[j+1].second - vec[j+1].first) -
                        (vec[j+1].second - vec[j].first);	
        ...
        if (flag == 2)
        {
            vec[minIndex].second = vec[minIndex+1].second;
            for (j=minIndex+1; j<vectorSize-1; j++)
            {
                vec[j] = vec[j+1];
            }
            vec[j] = make_pair(-1,-1);
        ...
        {
            for (j=minIndex; j<vectorSize-1; j++)
            {
                vec[j] = vec[j+1];
            }
            vec[j] = make_pair(-1,-1);
    ...
    {
        profit += vec[i].second - vec[i].first;
    }
...
{	
	...
	// Resource
	// Used my own Implementation
	// IB https://www.interviewbit.com/problems/best-time-to-buy-and-sell-stock-atmost-b-times/
	// LC https://leetcode.com/problems/best-time-to-buy-and-sell-stock-iv/submissions/
	// GFG https://www.geeksforgeeks.org/maximum-profit-by-buying-and-selling-a-share-at-most-k-times/

	// Explanition
	// First we find out the maximim profit with as many tranactions as possible
	// This is stored in a vector <pair>
	// Next we reduce our number of transactions to the amount as required in the Q
	// For reducing we remove those pairs that contribute the least
	// This is done in two particular ways
	// 1. We can remove a pair altogether {2,6} & {0,3}
	//	{0,3} contributes least remove it
	// 2. We could combine two pairs. In this case if conbining is possible
	//	Loss is calculated as profit of two original pairs - profit after combining
	// 	EG {1,7}, {2,9}, {0,9}
	//  Cobine first two and hence they become {1,9} {0,9}
	//
	// Combining works becuase prices are stated in order. Buying cheaper first
	//  and simply selling later is always an option. There is no need to sell
	//  at first sign of price drop
